...
class SolutionInfo
{
	...
	//	<< endl;
public:
	int run_cnt, mm, iteration, given_result_improve, result_improve;
	double obj, tm, tct;
	SolutionInfo(int _rc, double _tct, double _obj, int _mm, int _iter, double _tm, int _gri, int _ri)
...
class Analyze {
public:
	Analyze(string,string, string, string, string, int);
	void total_info();
	void group_info();
	void para_setting();
	double f[2], tct[2], d[2], h[2], t[2];
	int cmp_give_result_total_cnt[2][3], set_ins_cnt[2],imp_ins_cnt[2][3][3][2][2], ins_cnt[2][3][3][2][2];	// set,n,m,p,d
	double optfound_pct[2][10][4], avg_rel_error[2][3][3][2][2], avg_hit_ratio[2][3][3][2][2],
		avg_tct[2][3][3][2][2], avg_f[2][3][3][2][2], avg_t[2][3][3][2][2];
	enum CM {IMPROVED,EQUAL,NONIMPROVED};	// compare mode
	enum BM {MIN,AVG,MAX};
	enum PM {OB,BB,P1,P2,D1,D2,M1,M2,M3,N1,N2,N3};	// parameter mode
	enum JM { HR_JUDGE, DEV_JUDGE, TIME_JUDGE, OBJ_JUDGE };	// judge mode
	string str_pm[12]={ "OB","BB","P1","P2","D1","D2","M1","M2","M3","N1","N2","N3" };
	int whe_save_each_file_result;
private:
	string fnr,fnw,fnwt,fnr_best,fn;
...
Analyze::Analyze(string _fn, string _fnr, string _fnw,string _fnwt,string _fnr_best,int _ws):
	fn(_fn),fnr(_fnr),fnw(_fnw),fnwt(_fnwt),fnr_best(_fnr_best),whe_save_each_file_result(_ws)
{
	ifstream ifs(fnwt);
	if (!ifs.is_open())
	...
	while (getline(ifs, strline))
	{
		...
		else
		{
			sol_info_vec->push_back(new SolutionInfo(stoi(fields_vec[0]),//run_cnt
				stod(fields_vec[1]),//tct
				stod(fields_vec[2]),//obj
				stoi(fields_vec[3]),//mm
				stoi(fields_vec[4]), stod(fields_vec[5]),//iter, tm
...
void Analyze::para_setting()
{
	...
	for (int ps = OB; ps <= BB; ps++)
	{
		f[ps] /= set_ins_cnt[ps];
		d[ps] /= set_ins_cnt[ps], d[ps] *= 100;
		h[ps] /= set_ins_cnt[ps], h[ps] *= 100;
		t[ps] /= set_ins_cnt[ps];
		for (int para = PM::P1; para <= PM::N3; para++)
		{
			...
			{
				if (para <= PM::D2)
					optfound_pct[ps][para][judm] /= (set_ins_cnt[ps] / 2);
				else
					optfound_pct[ps][para][judm] /= (set_ins_cnt[ps] / 3);
				if (judm == JM::DEV_JUDGE || judm == JM::HR_JUDGE)
					optfound_pct[ps][para][judm] *= 100;
			}
		}
	}
	ofst << "fnr \tstr_jm\t"
...
void Analyze::group_info()
{
	ofst << "*" << fn << endl
		<< "set	p	d	m	n	avg_tct	avg_f	avg_t	imp_ins_cnt	avg_rel_error	avg_hit_ratio"
		<< endl;
	for (vector<InstanceInfo*>::iterator iter = insinfo_vec.begin();
		iter != insinfo_vec.end(); iter++)
	{
		cout << (*iter)->filename << ", " << (*iter)->m << ", " << (*iter)->n << ", "
			<< (*iter)->opt_obj_given << ", " << (*iter)->opt_obj_real << ", "
			<< (*iter)->rand_seed << ", " << (*iter)->sol_info_vec.size() << endl;
		double obj[3], tctm[3], iteration[3], tm[3];
		int hit_cnt = 0, cmp_give_result_cnt[3] = { 0,0,0 }, cmp_real_result_cnt[3] = { 0,0,0 };
		for (vector<SolutionInfo*>::iterator iter_sol = (*iter)->sol_info_vec.begin();
			iter_sol != (*iter)->sol_info_vec.end(); iter_sol++)
		{
			if (iter_sol == (*iter)->sol_info_vec.begin())
			{
				obj[MIN] = obj[AVG] = obj[MAX] = (*iter_sol)->obj;
				tctm[MIN] = tctm[AVG] = tctm[MAX] = (*iter_sol)->tct;
				iteration[MIN] = iteration[AVG] = iteration[MAX] = (*iter_sol)->iteration;
				tm[MIN] = tm[AVG] = tm[MAX] = (*iter_sol)->tm;
			}
			else
		...
		obj[AVG] /= (*iter)->sol_info_vec.size();
		tctm[AVG] /= (*iter)->sol_info_vec.size();
		iteration[AVG] /= (*iter)->sol_info_vec.size();
		tm[AVG] /= (*iter)->sol_info_vec.size();
		if (whe_save_each_file_result)
		...
		if ((set_index == OB && (*iter)->n == 8) || (set_index == BB && (*iter)->n == 20))
			...
			n_para = N3;
		n_para -= N1, m_para -= M1, p_para -= P1, d_para -= D1;
		ins_cnt[set_index][n_para][m_para][p_para][d_para] += 1;
		imp_ins_cnt[set_index][n_para][m_para][p_para][d_para] += (hit_cnt > 0 ? 1 : 0);
		avg_tct[set_index][n_para][m_para][p_para][d_para] += tctm[AVG];
		avg_f[set_index][n_para][m_para][p_para][d_para] += obj[AVG];
		avg_t[set_index][n_para][m_para][p_para][d_para] += tm[AVG];
		avg_rel_error[set_index][n_para][m_para][p_para][d_para] += (obj[AVG] - obj[MIN]) / obj[MIN];
		avg_hit_ratio[set_index][n_para][m_para][p_para][d_para] += ((double)hit_cnt / (*iter)->sol_info_vec.size());
		set_ins_cnt[set_index] += 1;
	}
	for (int ps = PM::OB; ps <= PM::BB; ps++)
	{
		for (int pp = PM::P1; pp <= PM::P2; pp++)
		{
			for (int pd = PM::D1; pd <= PM::D2; pd++)
			{
				for (int pm = PM::M1; pm <= PM::M3; pm++)
				{
					for (int pn = PM::N1; pn <= PM::N3; pn++)
					{
						int ps1 = ps - OB, pp1 = pp - P1, pd1 = pd - D1, pm1 = pm - M1, pn1 = pn - N1;
						avg_f[ps1][pn1][pm1][pp1][pd1] /= ins_cnt[ps1][pn1][pm1][pp1][pd1];
						avg_tct[ps1][pn1][pm1][pp1][pd1] /= ins_cnt[ps1][pn1][pm1][pp1][pd1];
						avg_t[ps1][pn1][pm1][pp1][pd1] /= ins_cnt[ps1][pn1][pm1][pp1][pd1];
						avg_rel_error[ps1][pn1][pm1][pp1][pd1] = 100 * avg_rel_error[ps1][pn1][pm1][pp1][pd1] / ins_cnt[ps1][pn1][pm1][pp1][pd1];
						avg_rel_error[ps1][pn1][pm1][pp1][pd1] = fabs(avg_rel_error[ps1][pn1][pm1][pp1][pd1]) <= MIN_EQUAL ?
							fabs(avg_rel_error[ps1][pn1][pm1][pp1][pd1]) : avg_rel_error[ps1][pn1][pm1][pp1][pd1];
						avg_hit_ratio[ps1][pn1][pm1][pp1][pd1] = 100 * avg_hit_ratio[ps1][pn1][pm1][pp1][pd1] / ins_cnt[ps1][pn1][pm1][pp1][pd1];
						ofst << str_pm[ps] << "\t" << str_pm[pp] << "\t" << str_pm[pd] << "\t" << str_pm[pm] << "\t" << str_pm[pn] << "\t"
							<< avg_tct[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< avg_f[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< avg_t[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< imp_ins_cnt[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< avg_rel_error[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< avg_hit_ratio[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< endl;
						cout << str_pm[ps] << "\t" << str_pm[pp] << "\t" << str_pm[pd] << "\t" << str_pm[pm] << "\t" << str_pm[pn] << "\t"
							<< avg_tct[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< avg_f[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< avg_t[ps1][pn1][pm1][pp1][pd1] << "\t"
							<< imp_ins_cnt[ps1][pn1][pm1][pp1][pd1] << "\t"
...
void Analyze::total_info()
{
	//if (whe_input_table_head)
		ofst << "*fnr \t"
		<< "tct[OB] \t f[OB] \t d[OB] \t h[OB] \t t[OB] \t"
		<< "cmp_give[OB][IMPROVED] \t"
		<< "cmp_give[OB][EQUAL] \t"
		<< "cmp_give[OB][NONIMPROVED] \t"
		<< "tct[BB] \t f[BB] \t d[BB] \t h[BB] \t t[BB] \t"
...
{
	char *rgv_linux[] = { "",	//0
		"_fn", "pms_eca0_am1_cl5_dc0_ls3_ox2_pu1_p20_np10_itr2000_ptr69_rm9_alpha30_beta60_r1_r20",	//1,2
		...
		"_whe_save_each","0",		// whether save the results of each file	
		"_alg", "2"	// invoke total_information function, group info, or per parameter function
	};
	char *rgv_win[] = { "",	//0
		"_fn", "pms_eca_am1_cl5_dc0_ls3_ox2_pu1_p20_np10_itr2000_ptr25_rm9_alpha30_beta60_r1_r20",	//1,2
		"_if", "instance\\BB_Problem_BestSolution\\",	//3,4	
		"_of", "results\\",	// output file path
		"_efn", "_analyze0",	// each output file name, append to the source file, trunc mode
		"_tfn", "total_information0", // total information of all the files, append mode
	...
	Analyze *an = new Analyze(argv_map.at("_fn"),fnr, fnw, fnwt, fnr_best, stoi(argv_map.at("_whe_save_each")));
	if(stoi(argv_map.at("_alg"))==0)
		an->para_setting();
	else if (stoi(argv_map.at("_alg")) == 1)
		an->group_info();
	else if (stoi(argv_map.at("_alg")) == 2)
		an->total_info();
#ifdef _WIN32
	system("pause");
#endif	
