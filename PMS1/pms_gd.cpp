...
private:
	...
	vector<vector<dete_type>> d, dg;	// deterioration effect, d depends on machines, dg depends on jobs
...
PMS::PMS(string file_input, string file_output, int _sol_num) :sol_num(_sol_num)
{
	...
	dg.resize(m + 1, vector<proc_type>(n + 1, 0));
	...
	dg[1].assign(d[1].begin(), d[1].end());	// the job dependent deteriorating effect is the same as 
	// the machine dependent deteriorating effect on the first machine
	// calculate r
	for (int i = 1; i <= m; i++)
	{
		for (int j = 1; j <= n; j++)
...
void PMS::check_solution(int si)
{
	...
	for (int i = 1; i <= m; i++)
	{
		...
		for (int j = 1; j < s[si][i].size(); j++)
		{
			cl += (p[i][s[si][i][j]] + dg[1][s[si][i][j]] * cl) / ql;
...
void PMS::calculate_completion_time(int si, int mach_index)
{
	for (int i = 1; i < s[si][mach_index].size(); i++)
	{
		...
		c[si][mach_index][i] = c[si][mach_index][i - 1] + (p[mach_index][s[si][mach_index][i]] + dg[1][i-1] * c[si][mach_index][i - 1]) / q[si][mach_index][i];
	}
}
	...
	else
	{
		if (position == 0)
		{
			...
			while (position < s[si][mach_index].size()
				...
				position += 1;*/
			proc_type min_ct = DBL_MAX;
			for (int i = 1; i <= s[si][mach_index].size(); i++)
			{
				// try to insert job after position i which begins at 0
				proc_type qi, ct;
				if (i != s[si][mach_index].size())
					qi = q[si][mach_index][i];
				else
					qi = (1 - d[mach_index][s[si][mach_index][i - 1]])*q[si][mach_index][i - 1];
				ct = c[si][mach_index][i - 1] + (p[mach_index][job] + dg[1][job] * c[si][mach_index][i - 1]) / qi;
				for (int j = i; j < s[si][mach_index].size(); j++)
				{
					if (j == i)
						qi *= (1 - d[mach_index][job]);
					else
						qi *= (1 - d[mach_index][j - 1]);
					ct = ct + (p[mach_index][s[si][mach_index][j]] + dg[1][s[si][mach_index][j]] * ct) / qi;
				}
				if (min_ct - ct > MIN_EQUAL)
				{
					min_ct = ct;
					position = i;
				}
		...
		if (position == 0)
		{
			q[si][mach_index].insert(q[si][mach_index].begin() + position, 1);
			c[si][mach_index].insert(c[si][mach_index].begin() + position, p[mach_index][s[si][mach_index][position]]);
		}
		else
		{
			...
			c[si][mach_index].insert(c[si][mach_index].begin() + position, c[si][mach_index][position - 1] +
				(p[mach_index][s[si][mach_index][position]] + dg[1][s[si][mach_index][position]] * c[si][mach_index][position - 1]) / q[si][mach_index][position]);
		}
		for (int i = position + 1; i < s[si][mach_index].size(); i++)
		{
			q[si][mach_index][i] = (1 - d[mach_index][s[si][mach_index][i - 1]])*q[si][mach_index][i - 1];
			c[si][mach_index][i] = c[si][mach_index][i - 1] + (p[mach_index][s[si][mach_index][i]] + dg[1][s[si][mach_index][i]] * c[si][mach_index][i - 1]) / q[si][mach_index][i];
		}
	}
}
	...
	if (pos_add == 0)
	{
		...
		for (int i = 1; i <= s[si][mach_index].size(); i++)
		{
			...
			if (i != s[si][mach_index].size())
				...
				qi = (1 - d[mach_index][s[si][mach_index][i - 1]])*q[si][mach_index][i - 1];
			ct = c[si][mach_index][i - 1] + (p[mach_index][job_add] + dg[1][job_add] * c[si][mach_index][i - 1]) / qi;
			for (int j = i; j < s[si][mach_index].size(); j++)
			{
				if (j == i)
					qi *= (1 - d[mach_index][job_add]);
				else
					qi *= (1 - d[mach_index][j - 1]);
				ct = ct + (p[mach_index][s[si][mach_index][j]] + dg[1][s[si][mach_index][j]] * ct) / qi;
			}
